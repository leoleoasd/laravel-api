...
class ErrorHandler extends ExceptionHandler
{
    public function render($request, Exception $exception)
    {
        ...
        $r = ResponseJar::make($exception->data ?? [], $exception->errorCode ?? -1, ('Excecption' != get_class($exception) ? get_class($exception) : '').' '.$exception->getMessage(), $exception->statusCode ?? 500,
                [
                    'request' => $request->all(),
